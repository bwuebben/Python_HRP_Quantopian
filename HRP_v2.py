...
def getRecBipart(cov, sortIx):
    ...
    while len(cItems) > 0:
        cItems = [i[j:k] for i in cItems for j, k in ((0, round(len(i) / 2)),
                                                      (round(len(i) / 2), len(i))) if len(i) > 1]  # bi-section
...
def hrpMC(numIters=10000, nObs=520, size0=5, size1=5, mu0=0, sigma0=1e-2,
    ...
    result=pd.concat([df0, df1, df1 / df1['getHRP'] - 1], axis=1)
    print(result)
    return stats
...
sns_plot=sns.clustermap(rets.corr())
sns_plot.savefig("HeatMapCorrelations.png")
...
#savefig("Time series of cumulative log returns")
...
fig.tight_layout()
fig.savefig("Performance_Graphs.png")
